...
\subsubsection{구성 요약}
AirSim (\emph{producer})은 IMU 데이터를 Shared Memory에 
\emph{write}(\SI{1}{kHz})하고, SIL 제어기 (\emph{consumer})는 이를 
\emph{read}하여 Mahony–Attitude P–Rate PI+D 루프를 거쳐 
PWM 신호를 \SI{400}{Hz}로 \emph{write}한다.  
AirSim은 해당 PWM 데이터를 \emph{read}하여 모터 모델에 적용한다.  

두 모듈은 \textbf{별도 프로그램(프로세스)}으로 구동되며, Shared Memory는 
AirSim이 생성·소유하고 SIL 제어기가 attach하여 read/write한다  
(\emph{IMU: AirSim→SIL, PWM: SIL→AirSim}).

\subsubsection{실행 절차}
프로그램 실행 시 내부적으로 다음 절차가 순차적으로 수행된다.
\begin{enumerate}
  \item AirSim 모듈이 초기화되며, 월드 및 기체 파라미터를 고정한 후 시뮬레이터의 시간 및 물리 \(\Delta t\) 일관성을 검증한다.
  \item SIL 제어기가 시작되어 메인 제어 스레드의 우선순위를 상향하고, 타이머 분해능을 \SI{1}{ms}로 설정한다.
  \item Shared Memory가 생성 및 연결되며, IMU(센서) 세그먼트와 PWM 세그먼트 간의 producer/consumer 상태를 확인한다.
  \item write/read 카운터가 주기적으로 증가하는지 모니터링하고, drop 또는 overwrite 비율이 0(또는 \(<\!0.5\%\))임을 점검한다.
  \item 제어 루프 주파수를 확인한다. IMU 입력은 약 \SI{1}{kHz}, PWM 출력은 \SI{400}{Hz}이며, 허용 오차는 \(\pm\)\SI{5}{\%} 이내여야 한다.
  \item 워밍업 \SI{10}{s} 이후 원시 IMU, 추정 자세, 제어 명령, PWM, 타임스탬프를 동기 기록한다.
  \item 실험 시나리오(S1--S4)에 따라 동일 초기 조건에서 순차 실행하며, 각 러닝별 로그 및 상태를 저장한다.
  \item 종료 시 로그 무결성을 검사하고 (타임스탬프 단조 증가, 샘플 수, 누락률), 이상이 없을 경우 분석 스크립트를 통해 지표를 산출한다.
\end{enumerate}
