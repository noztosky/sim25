...
세부 파라미터 값 및 실험 조건은 표~\ref{tab:key_params_xe}를 참조한다. 
...
\begin{itemize}
  \item 각 스텝마다 자이로 측정값을 이용해 쿼터니언을 \(\Delta t\) 간격으로 적분하고, 수치 오차 누적을 방지하기 위해 정규화를 수행한다.
  \item 가속도 크기가 중력 크기(\(|a|\!\approx\!g\))에 근접한 구간에서만 가속도 기반 보정 게인을 적용하며, 이 신뢰도(weight)는 시상수 \(\tau\)를 갖는 지수이동평균(EMA)으로 시간적으로 스무딩한다.
  \item 정지 또는 저가속 상태에서는 자이로 바이어스를 지수이동평균 형태로 온라인 추정하여, 장기 드리프트를 점진적으로 보정한다 (online bias tracking).
\end{itemize}
...
\subsection{시나리오 정의}
본 연구에서는 필터의 강건성과 외란 내성 평가를 위해 다음의 4가지 시나리오를 구성하였다.
\begin{enumerate}[label=S\arabic*]
  \item \textbf{Hover}: 외란이 없는 정지 호버 상태로, 필터의 정상상태 안정성과 잔류 진동 수준을 평가한다.
  \item \textbf{Gyro Bias}: 자이로 바이어스(\(\pm0.02\!\sim\!0.05\,\si{rad/s}\))를 인위적으로 주입하여, 오프셋 보정 및 장기 안정성을 검증한다.
...
세부 반복 횟수와 분석 윈도우는 섹션~4.6(프로토콜 및 지표)에 따른다.
...
\midrule
SHM & 78383.0 & 100.0 & 100.0 & 400.0 \\
RPC & 15516.0 & 100.0 & 100.0 & 340.0 \\
\bottomrule
\end{tabular}
\end{table}

수치 값의 정의 및 적용 위치는 표~\ref{tab:key_params_xe}를 따른다.
