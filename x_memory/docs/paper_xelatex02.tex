...
\node[node] (as_sens) at (-2.5,-0.30) {Sensors\\ACC, GYRO, BARO, MAG};
...
\node[node,     minimum width=5.0cm]    (fc_sens)   at (-2.3,   -3.9) {Sensors\\ACC, GYRO, BARO, MAG};
...
\subsection{AirSim 센서(ACC, GYRO, BARO, MAG)}
AirSim의 가속도계(accelerometer)와 자이로스코프(gyroscope) 센서는 운영체제(OS)의 공유 메모리(IPC) 세그먼트에 \emph{producer} 역할로 데이터를 기록하며, 각 레코드는 지연(latency)을 최소화하기 위해 고해상도 타임스탬프(high-resolution timestamp)를 포함한다. 또한 기압계(barometer)와 자기계(magnetometer) 데이터 역시 동일한 방식으로 송출되어, SIL 제어기 측에서 일관된 시각 기준으로 소비된다.

\subsection{Shared Memory (OS IPC)}
공유 메모리 구간은 단일 생산자–단일 소비자(SPSC, Single Producer Single Consumer) 링버퍼 구조로 구현되며, 비블로킹 접근(non-blocking access), zero-copy 전달, 고해상도 시계(high-resolution clock) 동기화를 통해 약 1 kHz의 텔레메트리를 안정적으로 제공한다. \emph{Producer}는 AirSim(IMU write)이고, \emph{Consumer}는 SIL 제어기(IMU read)로 역할이 구분되며, lock-free SPSC 인터페이스를 통해 데이터를 교환한다.

\subsection{SIL 제어기 센서}
SIL 제어기는 Shared Memory로부터 IMU 스트림을 \emph{consumer} 역할로 비블로킹 방식으로 읽어(state estimation 및 제어기 입력으로) 사용한다. 이를 통해 실제 비행제어기(Flight Controller, FC)와 동일한 센서 주파수 및 타이밍 조건에서 알고리즘을 검증할 수 있다.
...
이륙 후 약 3초 동안의 평균 오차 $(e_x, e_y)$를 \SI{80}{\micro\second\per rad}의 변환율로 PWM 명령치로 변환하고, 이를 $\pm\SI{50}{\micro\second}$ 범위로 포화시킨 목표값을 0.8초 동안 선형 램프-인(ramp-in) 방식으로 적용하였다(믹서 이전 단계). 또한 이산 스위치(discrete switch) 대신 시간상수 $\tau\!\approx\!\SI{0.25}{s}$의 1차 지수이동평균(Exponential Moving Average, EMA)을 이용하여 가중치 $w$를 갱신하고, 오차 합성식 $e=(1-w)e_{\mathrm{est}}+we_{\mathrm{gt}}$를 적용함으로써 단계 입력(step input)을 완화하였다.

\subsection{ROTOR PWM}
SIL 제어기는 컨트롤러에서 생성된 PWM 신호를 \SI{400}{Hz} 주기로 공유 메모리(Shared Memory)에 \emph{producer} 역할로 기록하며, AirSim의 로터(ROTOR)는 이를 \emph{consumer} 역할로 읽어 추력(thrust)과 토크(torque)를 생성한다. Slew rate 제한과 출력 클램프(clamp)는 SIL 제어기 측에서 적용되어 PWM의 급격한 변화율을 완화한다.


\section{실험 및 결과}
\subsection{실험환경}
\begin{itemize}
  \item \textbf{OS 및 플랫폼:} Windows 11, AirSim (월드 및 기체 파라미터 고정)
  \item \textbf{I/O 경로:} 프로세스 내 공유 메모리(in-process shared memory), IMU 약 \(1~\mathrm{kHz}\), PWM \(400~\mathrm{Hz}\)
  \item \textbf{실행 설정:} 스레드 우선순위 상향 및 타이머 분해능(timer resolution) \(1~\mathrm{ms}\)
\end{itemize}


\subsection{시스템 구성 및 실행 절차}
\subsubsection{구성 요약}
AirSim(\emph{producer})은 IMU 데이터를 공유 메모리(Shared Memory)에 약 \(1~\mathrm{kHz}\)로 write하며, SIL 제어기(\emph{consumer})는 이를 read하여 Mahony–Attitude P–Rate PI+D 제어 루프를 수행한 후, PWM 신호를 \SI{400}{Hz}로 write한다. AirSim은 해당 PWM을 read하여 모터 모델에 적용한다. 두 모듈은 \textbf{별도 프로그램(프로세스)}으로 구동되며, Shared Memory는 AirSim이 생성 및 소유하고, SIL 제어기가 attach하여 read/write를 수행한다(\emph{IMU: AirSim→SIL, PWM: SIL→AirSim}).

\subsubsection{실행 절차}
\begin{enumerate}
  \item AirSim 실행(월드 및 기체 파라미터 고정) 후 시뮬레이터 시간 및 물리 \(\Delta t\) 일관성을 확인한다.
  \item SIL 제어기 실행: 메인 제어 스레드의 우선순위를 상향하고, 타이머 분해능(timer resolution)을 \SI{1}{ms}로 설정한다.
  \item Shared Memory 연결 확인: IMU(센서) 세그먼트와 PWM 세그먼트에 대해 producer/consumer 연결 여부를 점검한다.
  \item 카운터 모니터링: write/read 누적 카운터가 주기적으로 증가하고, drop 또는 overwrite 비율이 0 (또는 \(<0.5\%\))임을 확인한다.
  \item 루프 주파수 점검: IMU 입력은 약 \SI{1}{kHz}, PWM 출력은 \SI{400}{Hz}이며, 허용 오차는 \(\pm5\%\) 이내로 유지되어야 한다.
  \item 워밍업(약 \SI{10}{s}) 이후 로그 수집을 시작한다. 수집 항목은 원시 IMU, 추정 자세, 제어 명령, PWM, 타임스탬프이며, 이들을 동기 기록한다.
  \item 실험 시나리오(S1–S4)를 동일한 초기 조건에서 연속 실행하고, 각 실험 러닝마다 실행 화면을 캡처하여 저장한다.
  \item 종료 시 로그 무결성 검사를 수행한다. 타임스탬프의 단조 증가, 샘플 수, 누락율을 확인하고, 분석 스크립트를 통해 최종 지표를 산출한다.
\end{enumerate}
