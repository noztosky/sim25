...
\begin{titlepage}
\onecolumn
\noindent{\fontsize{10}{12}\selectfont 사이버 물리 시스템 보안 특론}\par
\vspace*{0.28\textheight}
\centering
{\fontsize{18}{26}\selectfont AirSim 연동 고주파 제어 벤치마크 · 안정화 툴킷\par}
\vfill
...
\AuthorLine{ooo\textsuperscript{1\textdagger}· ooo\textsuperscript{2}· ooo\textsuperscript{3}}\par
...
\begin{figure}[htb]
  ...
  \caption{비행 중 주파수 측정 화면: IMU, MAG, BARO, LOC, PWM (Hz) 로그.}
\end{figure}
...
struct PwmCmd { 
  int64_t ts_ns;             // timestamp [ns]
  uint16_t m[4];             // motor PWM outputs [µs]
};

SpscQueue<PwmCmd> pwmQ;      // single-producer, single-consumer queue

void onControlStep() {       // called at controller rate (e.g., 400 Hz)
  PwmCmd c{ nowNanos(), mixOutputs() };
  pwmQ.try_enqueue(c);       // non-blocking; may drop command if full
}
\end{lstlisting}


\noindent\textbf{PWM Consumer (AirSim)}
\begin{lstlisting}[language=C++, caption={PWM consumer pseudocode}, label=lst:pwm_consumer]
// Reads motor commands from shared memory and applies to rotor model
void onRotorUpdate() {
  PwmCmd c;
  if (pwmQ.try_dequeue(c))
    applyPwm(c.m);           // update motor model with latest PWM [µs]
}
\end{lstlisting}
...
\subsection{시나리오 정의}
본 연구에서는 필터의 강건성 및 외란 내성(robustness and disturbance rejection)을 평가하기 위해 
다음의 네 가지 시나리오를 구성하였다.
\begin{enumerate}[label=S\arabic*]
  \item \textbf{Hover}: 외란이 없는 정지 호버 상태로, 필터의 정상상태 안정성과 잔류 진동 수준을 평가한다.
  \item \textbf{Gyro Bias}: 자이로 바이어스(\(\pm0.02\!\sim\!0.05\,\si{rad/s}\))를 인위적으로 주입하여, 오프셋 보정 성능 및 장기 안정성을 검증한다.
  \item \textbf{Dynamic Accel}: \(0.3\!\sim\!0.8\,g\) 진폭, \(0.5\!\sim\!2\,\si{Hz}\) 주파수 범위의 가속도 외란을 인가하여, 가속도 기반 보정 루프의 외란 내성을 평가한다.
  \item \textbf{Aggressive Tilt}: \(20\!\sim\!35^{\circ}\) 범위의 급격한 자세 명령(램프 또는 스텝 입력)을 통해, 큰 각속도·가속도 조건에서의 필터 수렴 특성을 분석한다.
\end{enumerate}

모든 시나리오는 워밍업 \SI{10}{s} 이후 분석되며, 
세부 반복 횟수와 분석 윈도우는 섹션~\ref{sec:protocol_metrics}(프로토콜 및 지표)에 따른다.
...
\noindent\textbf{비교군/설정}: CAP=1 조건에서 동일 PC/월드/\(\Delta t\)로 RPC 경로와 Shared Memory(SPSC, zero-copy) 경로를 비교하였다.\\
...
\subsubsection{통신 경로 비교 요약 및 로그(External Virtual FC)}
아래 표는 RPC와 Shared Memory 경로의 측정 주파수를 요약하며(표~\ref{tab:rpc_shm_rates}), 이어지는 로그는 SHM 기반 루프의 발췌 예시이다. 워밍업 \SI{10}{s}를 제외한 동일 분석 윈도우에서 수집하였다.
