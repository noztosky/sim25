        ...
        virtual void update() override
        {
            ...
            if (xsim_) {
                ...
                if (dt_sec >= 1.0) {
                    ...
                    // SHM sampling (emit) cadence jitter (measured in ImuXsim)
                    int sample_hz = XlabUeMetrics::getLoopHz();
                    ...
                    Utils::log(Utils::stringf(
                        "imu: %.2f %.2f %.2f %.2f %.2f %.2f(%dHz, emit %d, overrun %d), mag: %.2f %.2f %.2f (%dHz) baro: %.2f (%dHz) loc: %.2f %.2f %.2f (%dHz) pwm: %d %d %d %d  (%dHz) | sample: %dHz jit(us) mean=%d p99=%d max=%d",
                        ...
                        sample_hz, jit_mean, jit_p99, jit_max));
                    last_log_tp_ = now;
                    pwm_read_count_sec_ = 0;
                    prev_overrun_total = cur_overrun_total;
