...
struct XSimRingData {
    ...
    std::atomic<uint64_t> telem_overrun_cnt; // producer-reported overrun (based on backlog)
    std::atomic<uint32_t> telem_tail_seq;    // last consumed seq
    SlotP pwm_slots[CAP];
...
public:
    ...
    bool server_create(const char* name){
        ...
        mem_->telem_overrun_cnt.store(0,std::memory_order_relaxed);
        mem_->telem_tail_seq.store(0,std::memory_order_relaxed);
    ...
    // Server-side publish
    void publish_telem(const XSimTelemetry& d){
        if(!mem_) return;
        uint32_t next = mem_->telem_head_seq.load(std::memory_order_relaxed) + 1;
        uint32_t tail = mem_->telem_tail_seq.load(std::memory_order_relaxed);
        uint32_t backlog = next - tail;
        if (backlog > CAPACITY) {
            // count incremental overwrite per write (avoid double-counting absolute backlog)
            mem_->telem_overrun_cnt.fetch_add(1ULL, std::memory_order_relaxed);
        }
        size_t idx = next % CAPACITY;
        mem_->telem_slots[idx].data = d;
        mem_->telem_slots[idx].seq = next;
        mem_->telem_head_seq.store(next, std::memory_order_release);
        telem_cnt_.fetch_add(1,std::memory_order_relaxed);
    ...
    void consume_telem(uint32_t& last_seq, Fn&& on_sample){ if(!mem_) return; uint32_t head = mem_->telem_head_seq.load(std::memory_order_acquire); if(head==last_seq) return; if(head - last_seq > CAPACITY){ last_seq = head - (uint32_t)CAPACITY; } for(uint32_t s = last_seq+1; s<=head; ++s){ size_t idx = s % CAPACITY; if(mem_->telem_slots[idx].seq==s){ on_sample(mem_->telem_slots[idx].data); telem_rx_cnt_.fetch_add(1,std::memory_order_relaxed); tick(telem_rx_start_, telem_rx_cnt_, telem_rx_hz_); } } last_seq = head; mem_->telem_tail_seq.store(last_seq, std::memory_order_release); }

    // Peek telemetry without advancing shared tail (for internal display/logging)
    template<typename Fn>
    void peek_telem(uint32_t& last_seq, Fn&& on_sample){ if(!mem_) return; uint32_t head = mem_->telem_head_seq.load(std::memory_order_acquire); if(head==last_seq) return; if(head - last_seq > CAPACITY){ last_seq = head - (uint32_t)CAPACITY; } for(uint32_t s = last_seq+1; s<=head; ++s){ size_t idx = s % CAPACITY; if(mem_->telem_slots[idx].seq==s){ on_sample(mem_->telem_slots[idx].data); } } last_seq = head; }
    ...
    uint64_t get_telem_overrun_total() const { return mem_ ? mem_->telem_overrun_cnt.load(std::memory_order_relaxed) : 0ULL; }
